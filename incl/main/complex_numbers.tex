...
\section{Intro}
In the following chapter these sources are used as background information \cite{complexpaul}, \cite{complexpurple}, \cite{complexnotebook}. In this chapter firstly the concept of complex numbers will be describes, secondly complex numbers in polar coordinates, and lastly complex numbers all with the corresponding properties of their mathematical operations.
\\  
\\
...
\\
\\
An example of a complex number could be: $z=7+3i$, where 7 is the real part and 3 is the imaginary part. If $\text{Re}\{z\}=0$ the number is said to be pure imaginary.  
\\
...
\caption{The complex plane with the number $z=7+3i$, it's conjugate $\bar{z}=7-3i$, and $|z|$}
\end{figure}
\noindent
...
\end{align*}
\end{theorem}
\input{incl/profs/ezt_diff_proof.tex}
\noindent
