...
\end{prof}
To fully understand the concept behind figure \ref{lpsol}, an example of finding the solution of a differential equation will be solved using the Laplace transform. First, a table with the results from the previous examples and proofs are inserted:
...
\end{align*}
The Laplace transform of the derivative is inserted from theorem \ref{lpderiv}, and the Laplace transform of $e^{5t}$ is inserted from equation \eqref{lpe}:
\begin{align*}
...
\end{align*}
Both sides are now divided by $s$ and the numerator on the right side is simplified:
\begin{align*}
F(s) = \dfrac{6-s}{s(s-5)}
\end{align*}
...
\end{align*}
This result can be tested by taking the derivative of $f(t)$ and inserting it on equation \eqref{lpexinieq}:
\begin{align*}
