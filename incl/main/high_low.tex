...
The cut-off value can be calculated using the following equation:
\begin{align*}
f_{cutoff}=\dfrac{1}{2 \pi RC}
\end{align*}
The cut-off value for a low-pass filter is when the bode plot is decreased by $3dB$, and for a high-pass filter it is when the bode plot is $3dB$ away from reaching zero. The cut-off point is also the point, where the filtering (both high- and low-pass filters) starts getting efficient. The definition of a cut-off frequency is when the effect in watts is halved. The amount of decibel can be calculated as follows: \cite[p. 596-597]{bcircuit}
...
\end{align*}
From the KVL in section REF something called a transfer function can be derived. This is defined as the ratio between voltage output and input. The expression from equation \eqref{number:db} can be rewritten and set on the same form as the transfer function. \\
...
\input{incl/main/hpflpf_code.tex}
